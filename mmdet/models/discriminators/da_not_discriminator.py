...
class DANotDiscriminator(DABaseDiscriminator):
    """RepPoint head.
    ...
    def __init__(self,
        ...
        super(DANotDiscriminator, self).__init__()
    def _init_layers(self):
        ...
        self.mse = nn.MSELoss()
        #self.gradreverse = GradReverse(1)    This is not a discriminator, only a weak classifier
    ...
    def forward_single(self, x):
        #dis_feat = self.gradreverse.apply(x) #This is not a discriminator, only a weak classifier
        for idx, dis_conv in enumerate(self.cls_domain):
            if idx == 2:
                x = dis_conv(x)
                break
            x = self.relu2(dis_conv(x))
        feat_dis_scores = self.sigmoid(x)
