...
data_s = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type=dataset_type,
            ann_file=data_root_s +
            'annotations/instancesonly_filtered_gtFine_train.json',
            img_prefix=data_root_s + 'leftImg8bit/train/',
            pipeline=train_pipeline)),
    val=dict(
        type=dataset_type,
        ann_file=data_root_s + 'annotations/instancesonly_filtered_gtFine_val.json',
...
data_t = dict(
    ...
    test=dict(
        type=dataset_type,
        ann_file=data_root_t + 'annotations/instancesonly_filtered_gtFine_val.json',
        #ann_file=data_root_t + 'annotations/instances_filtered_filtered_grFine_val.json',
        img_prefix=data_root_t + 'leftImg8bit/val/',
        pipeline=test_pipeline))
