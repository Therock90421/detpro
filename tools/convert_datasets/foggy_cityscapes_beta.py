...
def collect_files(img_dir, gt_dir, beta):
    ...
    for img_file in glob.glob(osp.join(img_dir, '**/*.png')):
        if img_file.endswith(suffix):
            namesplit = osp.split(img_file)
            inst_file = gt_dir + '/' + namesplit[1].split('_')[0] + '/' + '_'.join(namesplit[1].split('_')[:-4]) + '_gtFine_instanceIds.png'
            # Note that labelIds are not converted to trainId for seg map
            segm_file = gt_dir + '/' + namesplit[1].split('_')[0] + '/' + '_'.join(namesplit[1].split('_')[:-4]) + '_gtFine_labelIds.png'
...
def main():
    ...
    for beta in [0.005, 0.01]:
        if beta == 0.005:
            cityscapes_path = '/GOODDATA/lihaochen/DSS/mmdetection/data/foggy_cityscapes_weak'
        elif beta == 0.01:
            cityscapes_path = '/GOODDATA/lihaochen/DSS/mmdetection/data/foggy_cityscapes_median'
        ...
        for split, json_name in set_name.items():
            ...
            with mmcv.Timer(
                ...
                files = collect_files(
                    osp.join(img_dir, split), osp.join(gt_dir, split), beta)
