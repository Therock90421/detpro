...
if __name__ == "__main__":
    ...
    #arguments
    #----------
    cityscapes_dir = 'foggy_cityscapes'
    save_path = os.path.join(cityscapes_dir,"foggy_cityscapes_voc")
        ...
        for city in os.listdir(os.path.join(cityscapes_dir_gt, category)):
            ...
            for file in files:
                ...
                if relevant_file:
                    ...
                    xml_filepath = os.path.join(ann_dir, base_filename + '_leftImg8bit_foggy_beta_0.01.xml')
                    img_name = base_filename + '_leftImg8bit_foggy_beta_0.01.png'
                    img_path = os.path.join(cityscapes_dir, 'leftImg8bit', category, city,
                                            base_filename + '_leftImg8bit_foggy_beta_0.01.png')
                    ...
                    img_path = os.path.join(cityscapes_dir, 'leftImg8bit', category, city,
                                            base_filename + '_leftImg8bit_foggy_beta_0.02.png')
                    img_shape = plt.imread(img_path).shape
                    valid_files.append([img_path, img_name])
                    trainval_files.append(img_name[:-4]) if category == 'train' else test_files.append(img_name[:-4])
                    save_xml(img_path, img_shape, data, xml_filepath)
                    ...
                    count += 1
                    base_filename = os.path.basename(file)[:-21]
                    xml_filepath = os.path.join(ann_dir, base_filename + '_leftImg8bit_foggy_beta_0.005.xml')
                    img_name = base_filename + '_leftImg8bit_foggy_beta_0.005.png'
                    img_path = os.path.join(cityscapes_dir, 'leftImg8bit', category, city,
                                            base_filename + '_leftImg8bit_foggy_beta_0.005.png')
                    img_shape = plt.imread(img_path).shape
                    valid_files.append([img_path, img_name])
                    trainval_files.append(img_name[:-4]) if category == 'train' else test_files.append(img_name[:-4])
                    save_xml(img_path, img_shape, data, xml_filepath)
    ...
    with open(os.path.join(textfiles_savepath, 'target_trainval.txt'), 'w') as f:
